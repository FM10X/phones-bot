...
async def send_location(message: types.Message):
    chat_id = message.chat.id
    latitude = 41.26527557713472
    longitude = 69.20043760418582
...
async def make_order(mesage: Message):
    await mesage.answer("Bo'limni tanlang 👇", reply_markup=generate_category_menu())
...
async def add_product_cart(call: CallbackQuery):
    ...
    if insert_or_update_cart_product(cart_id, product[2], quantity, final_price):
        await bot.answer_callback_query(call.id, "Mahsulot qo`shildi")
    else:
...
Umumiy narxi: {final_price}\n\n'''
        ...
        text += f'''Umumiy maghsulot soni: {total_products}
Umumiy to`lash narxi: {total_price} 💴 💳'''
        ...
        await bot.send_message(chat_id, "Savat bo`m bo`sh")
...
async def delete_cart_product(call: CallbackQuery):
    ...
    await bot.answer_callback_query(call.id, text="Mahsulot o`chirildi")
...
Umumiy to`lash narxi: {total_price} 💴 💳'''
        ...
        await bot.send_invoice(
            ...
            currency='UZS',
            prices=[
                LabeledPrice(label="Umumiy narx", amount=int(total_price * 100)),
...
async def checkout(pre_checkout_query):
    await bot.answer_pre_checkout_query(pre_checkout_query.id,
                                        ok=True,
                                        error_message='Xatolik!!! Hisobingizni tekshiring 👝!!')

@dp.message_handler(content_types=['successful_payment'])
async def get_payment(message):
    chat_id = message.chat.id
    cart_id = get_user_cart_id(chat_id)
    await bot.send_message(chat_id, "To'lov muvaffiqiyatli yakunlandi ⚖")
